...
/// Two nested layouts, standing in for what a `VmLayout` derive emits.
static HEADER: TypeLayout = TypeLayout::new(&[
    crate::marshal::Field::new("seq", 0, 4, None),
    crate::marshal::Field::new("flags", 4, 1, None),
]);
static PACKET: TypeLayout = TypeLayout::new(&[
    crate::marshal::Field::new("kind", 0, 1, None),
    crate::marshal::Field::new("header", 4, 8, Some(&HEADER)),
    crate::marshal::Field::new("tag", 16, 8, None),
]);

/// `[push32 0, halt]`, the first push a placeholder for a field offset.
fn field_hole_artifact(path: &'static [&'static str]) -> Artifact<Le> {
    let mut code = Vec::new();
    encode::<Le>(Push32 { imm: 0 }.into(), &mut code).expect("encodes");
    encode::<Le>(Halt.into(), &mut code).expect("encodes");

    Artifact::new(
        code,
        FrameSize::default(),
        vec![Reloc::field_offset::<Le>(0, &PACKET, path)],
    )
}

/// A field hole finalizes to the offset the layout gives the path.
#[test]
fn a_field_offset_comes_from_the_layout() {
    let program = field_hole_artifact(&["header", "seq"])
        .finalize(&layout(64))
        .expect("finalizes");

    match disassemble(&program).first().copied() {
        Some(Instr::Push32(op)) => assert_eq!(op.imm, 4),
        other => panic!("expected a pushed offset, got {other:?}"),
    }
}

/// A path that names no field fails at finalize rather than pushing a guess.
#[test]
fn an_unknown_field_is_refused() {
    assert!(matches!(
        field_hole_artifact(&["header", "nope"]).finalize(&layout(64)),
        Err(NotFinal::UnknownField { .. })
    ));
}
