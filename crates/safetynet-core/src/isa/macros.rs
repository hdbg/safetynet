...
//! implementation, its arm in the [`Instr`] enum, and its assembly form.
//!
//! [`Op`]: super::Op
//! [`Instr`]: super::Instr
    ...
    ($(
        ...
        $(
            impl Op for $name {
                ...
                // and `blackboxify!` wraps every call and sub-expression of the
                // body in `black_box`, so the optimizer cannot merge or fold the
                // handlers back out — the interpreter's shape survives into the
                // binary instead of collapsing to the arithmetic it stands for.
                #[inline(always)]
                fn exec<B: ByteOrder>(&self, vm: &mut Vm<B>) -> Result<Flow, Trap> {
                    let $vm: &mut Vm<B> = vm;
                    let $exec_op: &Self = self;
                    ::safetynet_blackbox::blackboxify! { $exec }
                }
            }
        ...
        impl Instr {
            ...
            /// One instruction of every kind, operands left at their defaults.
            ///
            /// The anchor anything that has to be total over the instruction
            /// set — a printer, a parser, an emitter — is tested against. A new
            /// entry in the table appears here the moment it is declared, so a
            /// match that forgot it fails a test instead of shipping.
            pub fn one_of_each() -> Vec<Self> {
                vec![ $( Self::$name($name::default()), )* ]
            }
        }
